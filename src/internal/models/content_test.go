...
func testContentToManyNoticeFavorites(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ContentSlice{&a}
	if err = a.L.LoadNoticeFavorites(ctx, tx, false, (*[]*Content)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testContentToManyAddOpNoticeFavorites(t *testing.T) {
	var err error
...
var (
	contentDBTypes = map[string]string{`ID`: `int`, `UserID`: `int`, `CategoryID`: `int`, `Title`: `varchar`, `Description`: `text`, `ThumbnailURL`: `text`, `Author`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_              = bytes.MinRead
)
