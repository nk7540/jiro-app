...
func testFavoriteToManyNoticeFavorites(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadNoticeFavorites(ctx, tx, false, (*[]*Favorite)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFavoriteToManyAddOpNoticeFavorites(t *testing.T) {
	var err error
...
func testFavoriteToOneCommentUsingComment(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, favoriteDBTypes, true, favoriteColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Favorite struct: %s", err)
	}
	...
	if err = local.L.LoadComment(ctx, tx, false, (*[]*Favorite)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFavoriteToOneSetOpCommentUsingComment(t *testing.T) {
	var err error
...
func testFavoriteToOneRemoveOpCommentUsingComment(t *testing.T) {
	var err error
...
var (
	favoriteDBTypes = map[string]string{`ID`: `int`, `UserID`: `int`, `ContentID`: `int`, `CommentID`: `int`, `IsMost`: `tinyint`, `ToUserIds`: `varchar`, `ToCloseUsers`: `tinyint`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_               = bytes.MinRead
)
