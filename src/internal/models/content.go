...
var ContentWhere = struct {
	...
	Author:       whereHelperstring{field: "`content`.`author`"},
...
var (
	contentAllColumns            = []string{"id", "user_id", "category_id", "title", "description", "thumbnail_url", "author", "created_at", "updated_at"}
	contentColumnsWithoutDefault = []string{"user_id", "category_id", "title", "author", "created_at", "updated_at"}
	contentColumnsWithDefault    = []string{"id", "description", "thumbnail_url"}
...
func (o *Content) NoticeFavorites(mods ...qm.QueryMod) noticeFavoriteQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`notice_favorite`.`content_id`=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (contentL) LoadNoticeFavorites(ctx context.Context, e boil.ContextExecutor, singular bool, maybeContent interface{}, mods queries.Applicator) error {
	var slice []*Content
	var object *Content
	...
	query := NewQuery(
		...
		qm.WhereIn(`notice_favorite.content_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Content appropriately.
func (o *Content) AddNoticeFavorites(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*NoticeFavorite) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ContentID = o.ID
