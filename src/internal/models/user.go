...
type User struct {
	...
	Profile      string    `boil:"profile" json:"profile" toml:"profile" yaml:"profile"`
...
var UserWhere = struct {
	...
	Profile:      whereHelperstring{field: "`user`.`profile`"},
...
var (
	userAllColumns            = []string{"id", "uid", "status", "email", "nickname", "thumbnail_url", "profile", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"uid", "email", "nickname", "profile", "created_at", "updated_at"}
	userColumnsWithDefault    = []string{"id", "status", "thumbnail_url"}
...
func (o *User) Notices(mods ...qm.QueryMod) noticeQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (userL) LoadContents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`content`),
		qm.WhereIn(`content.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadFollowerFollows(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`follow`),
		qm.WhereIn(`follow.follower_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadFollowingFollows(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`follow`),
		qm.WhereIn(`follow.following_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadNotices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`notice`),
		qm.WhereIn(`notice.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on notice")
	}
	if err = results.Err(); err != nil {
...
// Sets related.R.User appropriately.
func (o *User) AddNotices(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Notice) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
